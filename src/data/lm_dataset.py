...
def build_dataset(data_file, tokenize_fn, group_fn):
    raw_dataset = load_dataset("parquet",data_files=datafile)["train"]
...
def make_lm_data_module(data_args,model_args,tokenizer):
    ...
    if data_args.codeswitch_ratio == 0:
        ...
        codeswitch_table = load_codeswitch_table(tokenizer,data_args.dict_file)
        data_collator = DataCollatorForLMDataset(tokenizre,data_args.codeswitch_ratio,codeswitch_table)
