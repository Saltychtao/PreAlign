...
from src.trainer_callback import DefaultFlowCallback
...
class Trainer:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step, loss_dict_step = self.training_step(model, inputs)
                ...
                if (
                    ...
                    for k in loss_dict_step:
                        if k not in tr_loss_dict:
                            tr_loss_dict[k] = loss_dict_step[k] / (1 + self.state.global_step - self._globalstep_last_logged)
                        else:
                            tr_loss_dict[k] += tr_loss_dict[k] / (1 + self.state.global_step - self._globalstep_last_logged)
                else:
                    tr_loss += tr_loss_step
                    for k in loss_dict_step:
                        if k not in tr_loss_dict:
                            tr_loss_dict[k] = loss_dict_step[k]
                        else:
                            tr_loss_dict[k] += loss_dict_step[k]
    ...
    def _maybe_log_save_evaluate(self, tr_loss_dict, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log:
            ...
            tr_loss_scalar_dict = {k: v.mean().item() for k,v in self._nested_gather(tr_loss_dict).items()}
            for k in tr_loss_dict:
                tr_loss_dict[k] -= tr_loss_dict[k]
            # tr_loss_scalar = self._nested_gather(tr_loss).mean().item()

            # reset tr_loss to zero
            # tr_loss -= tr_loss
            for k,v in tr_loss_scalar_dict.items():
                logs[k] = round(v / (self.state.global_step - self._globalstep_last_logged), 4)
            # logs["loss"] = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["learning_rate"] = self._get_learning_rate()

            self._total_loss_scalar += tr_loss_scalar_dict["loss"]
            self._globalstep_last_logged = self.state.global_step
            self.store_flos()
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss_dict = self.compute_loss(model, inputs)
            loss = loss_dict["loss"]
        ...
        loss_dict = {k: v.detach() / self.args.gradient_accumulation_steps for k,v in loss_dict.items()}
